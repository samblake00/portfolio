...
import { Cartography } from '@/features/cartography/cartography-view';

export default function SingleCartography({ cartography }: { cartography: Cartography }) {
  const [activeImage, setActiveImage] = useState<StaticImageData | string>(
    cartography.thumbnail
  );
  return (
    <div className="py-10">
      <motion.div
      ...
      >
        ...
        />
        <div className="absolute bottom-0 bg-black bg-opacity-50 h-40 w-full [mask-image:linear-gradient(to_bottom,transparent,black)]" />
      </motion.div>
      <div className="flex flex-row justify-center my-8 flex-wrap">
        {cartography.images.map((image: any, idx: number) => (
          <button
      ...
      <div className="flex lg:flex-row justify-between items-center flex-col mt-20">
        <Heading className="font-black mb-2 pb-1"> {cartography.title}</Heading>
      ...
      <div>
        <Paragraph className="max-w-xl mt-4">{cartography.description}</Paragraph>
      </div>
